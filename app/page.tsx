  ...
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-white/10 p-6 bg-white/5">
        ...
        <p className="text-sm text-slate-300">İndekste Türkiye kaynakları önceliklidir.</p>
      </section>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        {items.map((a) => (
          <a key={a.id} href={a.link} target="_blank" rel="noreferrer"
            ...
            <div className="text-xs text-slate-300 mb-2">{a.source} • {new Date(a.published).toLocaleString("tr-TR")}</div>
            <h2 className="text-lg font-medium">{a.title}</h2>
            {a.summary && <p className="mt-2 text-sm text-slate-200 line-clamp-3">{a.summary}</p>}
          </a>
        ))}
      </section>
    </div>
